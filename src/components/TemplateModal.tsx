...
export function TemplateModal() {
  const { isTemplateModalOpen, setIsTemplateModalOpen, editingTemplateId, setEditingTemplateId } = useAppStore();
  const categories = useLiveQuery(() => db.categories.toArray());
  
  const existingTemplate = useLiveQuery(
    () => editingTemplateId ? db.templates.get(editingTemplateId) : undefined,
    [editingTemplateId]
  );
  ...
  useEffect(() => {
    if (existingTemplate) {
      ...
      setDefaultTitle(existingTemplate.defaultTitle || '');
      setCategoryId(existingTemplate.categoryId || 'uncategorized');
      setContent(existingTemplate.content);
      setHighlights(existingTemplate.defaultHighlights || []);
    } else {
      setName('');
      setDefaultTitle('');
  ...
  }, [existingTemplate, isTemplateModalOpen]);
  ...
  const saveTemplate = async () => {
    ...
    if (editingTemplateId) {
      await db.templates.update(editingTemplateId, data);
    } else {
